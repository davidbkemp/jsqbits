describe('QState', function() {
    ...
    var qstate = jsquantum.QState.fromBits;
    ...
    describe('#hadamard', function() {
        it("applies the hadamard operation", function() {
            ...
            expect(x.amplitude('|001>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|010>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|011>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|100>')).toBeApprox(complex(1 / Math.sqrt(2), 0));
            expect(x.amplitude('|101>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|110>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|111>')).toBe(jsquantum.Complex.ZERO);
        });
    });
    ...
    describe('#hadamard', function() {
        it("is it's own inverse", function() {
            ...
            expect(x.amplitude('|010>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|011>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|100>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|101>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|110>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|111>')).toBe(jsquantum.Complex.ZERO);
        });
    ...
    describe('#rotateX', function() {
        it("rotates about the X axis", function() {
            ...
            expect(x.amplitude('|01>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|10>')).toBeApprox(complex(0, -Math.sin(Math.PI/8)));
            expect(x.amplitude('|11>')).toBe(jsquantum.Complex.ZERO);
        });
    ...
    describe('#rotateZ', function() {
        it("rotates about the Z axis (|0>)", function() {
            ...
            expect(x.amplitude('|1>')).toBe(jsquantum.Complex.ZERO);
        });
        it("rotates about the Z axis (|1>)", function() {
            var x = qstate('|1>').rotateZ(0, Math.PI/4);
            expect(x.amplitude('|0>')).toBe(jsquantum.Complex.ZERO);
    ...
    describe('Simple combination of hadamard and cnot', function() {
        it("results in a phase kick back", function() {
            ...
            expect(x.amplitude('|00>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|01>')).toBe(jsquantum.Complex.ZERO);
            expect(x.amplitude('|10>')).toBe(jsquantum.Complex.ZERO);
